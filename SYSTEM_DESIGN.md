...
graph TB
    subgraph Frontend["前端 (Vue 3 + Vite)"]
        direction TB
        FE_Pages["仪表盘 · 区域管理 · 网络平面类型管理 · IP 查找<br/>导入/导出 · 变更历史 · 区域详情(IP 分配)"]
        FE_Axios["Axios / REST API（Vite Proxy: 5173 → 8000）"]
        FE_Pages --> FE_Axios
    end

    subgraph Backend["后端 (FastAPI)"]
        direction TB
        Routers["Routers（API 路由层）<br/>regions · plane-types · allocations+lookup · excel<br/>change-logs · stats · backup"]
        Services["Services（业务逻辑层）<br/>region / plane_type / allocation / excel / change_log / backup<br/>· CIDR 重叠检测（Python ipaddress）<br/>· 变更日志自动记录<br/>· Excel 预览缓存（30 分钟 TTL）<br/>· 备份目标配置、手动备份、定时备份调度"]
        Models["Models（SQLAlchemy ORM）<br/>Region / NetworkPlaneType / RegionNetworkPlane /<br/>IPAllocation / ChangeLog / BackupConfig / BackupRecord"]
...
graph TB
    ...
    GHA --> Test["test-backend<br/>pytest 45 项<br/>SQLite 内存"]
