...
class RegionNetworkPlane(Base):
    ...
    plane_type_id = Column(String(36), ForeignKey("network_plane_types.id", ondelete="CASCADE"), nullable=False)
    ...
    # 父平面（自引用）
    parent = relationship("RegionNetworkPlane", remote_side="RegionNetworkPlane.id", back_populates="children")
    # 子平面列表（级联删除：删父平面时自动删子平面及其 IP 分配）
    children = relationship("RegionNetworkPlane", back_populates="parent", cascade="all, delete-orphan")
    # 此平面节点下的 IP 分配（级联删除：删平面时自动删关联分配）
    allocations = relationship("IPAllocation", back_populates="plane", cascade="all, delete-orphan")
